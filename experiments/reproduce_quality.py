...
agent_params = dict(
    ...
    QSM={ 
        "puzzle-3x3-play-sparse":       dict(actor_loss_type="qsm", inv_temp=3.0, alpha=30.),
        "scene-play-sparse":            dict(actor_loss_type="qsm", inv_temp=3.,  alpha=30.),
        "cube-double-play":             dict(actor_loss_type="qsm", inv_temp=3.0, alpha=10.),
        "antmaze-large-navigate":       dict(actor_loss_type="qsm", inv_temp=30., alpha=30.),
        "humanoidmaze-medium-navigate": dict(actor_loss_type="qsm", inv_temp=10., alpha=30.),

        "cube-triple-play":             dict(actor_loss_type="qsm", inv_temp=3.0, alpha=3.0),
        "cube-quadruple-play":          dict(actor_loss_type="qsm", inv_temp=1.0, alpha=10.),
        "puzzle-4x4-play-sparse":       dict(actor_loss_type="qsm", inv_temp=10., alpha=1. ),
        "antmaze-giant-navigate":       dict(actor_loss_type="qsm", inv_temp=30., alpha=30.),
        "humanoidmaze-large-navigate":  dict(actor_loss_type="qsm", inv_temp=10., alpha=30.),
    },
    DAC={ 
        "puzzle-3x3-play-sparse":       dict(actor_loss_type="dac", alpha=1.0),
        "scene-play-sparse":            dict(actor_loss_type="dac", alpha=1.0),
        "cube-double-play":             dict(actor_loss_type="dac", alpha=3.0),
        "antmaze-large-navigate":       dict(actor_loss_type="dac", alpha=0.3),
        "humanoidmaze-medium-navigate": dict(actor_loss_type="dac", alpha=1.0),

        "cube-triple-play":             dict(actor_loss_type="dac", alpha=0.3),
        "cube-quadruple-play":          dict(actor_loss_type="dac", alpha=1.0),
        "puzzle-4x4-play-sparse":       dict(actor_loss_type="dac", alpha=1.0),
        "antmaze-giant-navigate":       dict(actor_loss_type="dac", alpha=0.3),
        "humanoidmaze-large-navigate":  dict(actor_loss_type="dac", alpha=0.3),
    },
)
    ...
    for seed in [10001, 20002, 30003, 40004, 50005, 60006, 70007, 80008, 90009, 100010, 110011, 120012]:
        ...
        for env_name, domain in zip(env_names, domains):
            ...
            for method in methods:
                if method in ["QAM_EDIT", "QAM_FQL", "QAM", "REBRAC", "FEDIT", "FQL", "QSM", "DSRL", "BAM"]:
